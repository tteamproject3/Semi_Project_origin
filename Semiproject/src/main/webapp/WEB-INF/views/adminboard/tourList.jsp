...
<!-- PAGE TITLE 표시 바 -->
	...
	<section class="board">
		...
		<div class="board__bar">
			<ul class="board__nav">
				<li class="active"><a href = "/adminboard/tourList">여행지 관리</a></li>
		...
		<div id="board" class="board__content">
			...
			<ul class="content__list">
				<c:forEach var="vo" items="${list}">
					<li>${vo.tour_num}</li>
					<li>${vo.tour_id}</li>
					<li>${vo.tour_content}</li>
					<li>${vo.tour_road_name_addr}</li>
				</c:forEach>
			</ul>
			...
			<div class="board__bottom">
					...
					<c:if test="${pVO.nowPage>1 }">
						<!-- 이전 페이지가 있을 때  -->
						<li><a
							href="/adminboard/tourList?nowPage=${pVO.nowPage-1 }<c:if test="${pVO.searchWord!=null }">&searchKey=${pVO.searchKey }&searchWord=${pVO.searchWord }</c:if>
					...
					<c:forEach var="p" begin="${pVO.startPage }" end="${pVO.startPage + pVO.onePageCount - 1 }">
						...
						<c:if test="${p<=pVO.totalPage }">
							...
							>
							<a
								href="/adminboard/tourList?nowPage=${p }
